...
loop terminates (reaches step 6, Accept) when every claim has a verdict:
`∀c ∈ C, verdict(c) ∈ {holds, falsified}`.
...
3. **Verify.** Adjudicate each claim in `C` against `I`. A mechanical deterministic
   ...
   with a counterexample kept permanently.
...
- **verdict**: the outcome of checking a claim: `proven`, `holds`,
  `documented`, `falsified`, `skipped`. [`evidence-ladder.md`](evidence-ladder.md).
